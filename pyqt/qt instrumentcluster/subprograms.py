...
def pin_changed_callback(channel):
    ...
    if channel == 3:
        update_BlinkerLeft(output)
    elif channel == 6:
        update_BlinkerRight(output)
    elif channel == 13:
        update_HighBeam(output)

    # channels are 3, 6 and 13, which are 
    # left blinker, right blinker and high beam
    

# Functions to update the pinchange callback for QLabel
        
def update_HighBeam(status):
    global highbeam_state
    highbeam_state = status
    highbeam_label.setText(f"High Beam: {highbeam_state}")

def update_BlinkerLeft(status):
    global blinkerleft_state
    blinkerleft_state = status
    blinkerleft_label.setText(f"Blinker Left: {blinkerleft_state}")
       
def update_BlinkerRight(status):
    global blinkerright_state
    blinkerright_state = status
    blinkerright_label.setText(f"Blinker right: {blinkerright_state}")
...
def send_data_and_calc_odo(odotime, gear_speed_rpm, status, sceneout, otherdata):
    ...
    output_list = [gear_speed_rpm[0], gear_speed_rpm[1], status[2], status[3], sceneout, dt_string, 
    ...
    output = [distanceinkm, newtime, output_list] # Output time for next loop and distance to add to the odo and trip.
    return output
