...
def update_rpm(RPM_PIN):
    ...
    while True:
        rpmdata = round(getrpm(RPM_PIN), 0)
        rpm_label.setText(f"RPM: {rpmdata}")
        time.sleep(0.01)

# Function to update the text of the QLabel for main loop
def updateMain():
    global outputlist
...
output_thread = threading.Thread(target=updateMain)
...
blinkerleft_thread.daemon = True
blinkerright_thread.daemon = True
